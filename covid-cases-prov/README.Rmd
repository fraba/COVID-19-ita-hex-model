...
reg_testing.df <- 
  dat.df %>%
  mutate(date = as.Date(data, format = "%Y-%m-%dT%H:%M:%S")) %>%
  select(codice_regione, denominazione_regione, date, nuovi_positivi, tamponi) %>%
  group_by(codice_regione, denominazione_regione) %>%
  arrange(date) %>%
  mutate(cases_ma7 = rollmean(nuovi_positivi, 7, align = 'left', fill = NA),
