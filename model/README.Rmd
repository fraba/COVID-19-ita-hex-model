...
ggsave(file = "../fig/comune_lombardy.pdf", 
       ggplot() +  
         scale_fill_viridis() +
         geom_sf(data = italy_comune.sf %>% 
                   dplyr::filter(COD_REG == 3)  
                 %>% st_transform(4326), 
                 colour = 'red', size = 0.3, fill = NA) +
         geom_sf(data = hex_ita_10km.sf %>% 
                   st_transform(4326), 
                 fill = NA, colour = "white", alpha = .5) +
...
# Neighbours (using communiting weighted network)
...
predictor_labels <- 
  c(dist_sd_lag4_diff17 = "past excess mortality",
    ...
    `dist_sd_lag4_diff17:perc_pop_lockdown` = "past excess mortality X perc. lockdown",
    `perc_pop_lockdown:neigh_weighted_mean` = "neighbouring excess mortality X perc.  lockdown",
    `dist_sd_lag4_diff17:neigh_weighted_mean` = "past excess mortality X neighbouring excess mortality",
    `dist_sd_lag4_diff17:perc_pop_lockdown:neigh_weighted_mean` = "past excess mortality X neighbouring excess mortality X  perc. lockdown")
```
