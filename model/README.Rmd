...
model_vars.df <- 
  model_vars.df %>% filter(day < as.Date("2020-03-16"))
...
model_vars.df$dist_sd_diff14[
  is.infinite(model_vars.df$dist_sd_diff14)] <- 0
model_vars.df$dist_sd_lag4_diff14[
  is.infinite(model_vars.df$dist_sd_lag4_diff14)] <- 0
...
hex_pop_lockdown$perc_mar8 <-
  hex_pop_lockdown$P1_mar8_lockdown / hex_pop_lockdown$P1

hex_pop_lockdown$perc_feb23[is.nan(hex_pop_lockdown$perc_feb23)] <- 0
hex_pop_lockdown$perc_mar8[is.nan(hex_pop_lockdown$perc_mar8)] <- 0
...
for (this_hex_id in unique(hex_pop_lockdown$hex_id[
  hex_pop_lockdown$perc_mar8>0])) {
  this_perc <- 
    hex_pop_lockdown$perc_mar8[hex_pop_lockdown$hex_id == this_hex_id]
...
model_vars_std.df <-
  ...
  dplyr::select(hex_id, day, perc_pop_lockdown,
        ...
        pm2p5_conc, pm10_conc, dust, no_conc, 
        P1, perc_over_74, pop_over_74, perc_work_outside_comune, 
...
pooled <- 
  lm(dist_sd_diff14 ~ 
       ...
       perc_pop_lockdown +
       median_density_km2 +  
       pm2p5_conc + pm10_conc + no_conc +
       perc_over_74 + gini_over74 + 
...
tidy(pooled) %>% 
  ...
  relabel_predictors(c(dist_sd_lag4_diff14 = "past excess mortality",
                       ...
                       pm2p5_conc = "PM2.5 concentration",
                       pm10_conc = "PM10 concentration",
                       no_conc = "nitrogen dioxide concentration",
                       perc_over_74 = "perc. over 74",
                       gini_over74 = "over 74 concetration",
                       ...
                       `dist_sd_lag4_diff14:neigh_weighted_mean:perc_pop_lockdown` = "past excess mortality X near exces mortality X lockdown")) +
...
funPlotGif <- function(day) {
  ...
  hex_ita_1km.sf$value <- this_dat$dist_sd[match(hex_ita_1km.sf$hex_id,
                                                 this_dat$hex_id)]
    hex_ita_1km.sf$value_cut <- cut(hex_ita_1km.sf$value, 
                                  breaks = rev(c(-128,-50,-25,-10,-5,-2, 2,5,10,25,50,128)),
                                  include.lowest = T)
