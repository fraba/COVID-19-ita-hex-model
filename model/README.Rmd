...
# Cases of Death (Provincial level, 2017)
...
```{r}
dat_health_status <- 
  read.csv("../healt_status_reg/DCCV_AVQ_PERSONE_19062020065748989.csv") %>%
    dplyr::filter(grepl("IT([A-Z])(\\d+|[A-Z])", ITTER107) &
                  !ITTER107 %in% c("ITFG","ITCD","ITD1","ITD2") & 
                  Misura == "per 100 persone con le stesse caratteristiche" &
                  TIME == 2018) %>%
...
```{r}
dat_icu <- 
  read.csv("../icu/C_17_dataset_96_0_upFile.csv",
           sep=";", fileEncoding = "ISO-8859-1") %>%
  dplyr::filter(Descrizione.disciplina == "TERAPIA INTENSIVA                       ")
  

sum(!dat_icu$Codice.Comune %in% sprintf("%06d", as.numeric(hex_ita_1km.sf$PROCOM)))
...
model_vars_std.df <-
  ...
  dplyr::select(hex_id, day, perc_pop_lockdown,
        ...
        cause_death_8, cause_death_7, cause_death_2_1_10, 
        cause_death_2_1_4, cause_death_2_1_8, 
        CHRONIC_DIAB, CHRONIC_HYP, CHRONIC_BRONC, CHRONIC_ALLER,
        smokers,
        hotel_beds,
...
tidy(pooled) %>% 
  ...
  relabel_predictors(c(dist_sd_lag4_diff14 = "past excess mortality",
                      ...
                      cause_death_8 = "cause death: respiratory (prov.)",
                      cause_death_7 = "cause death: circulatory (prov.)",
                      cause_death_2_1_10 = "cause death: breast cancer (prov.)",
                      cause_death_2_1_4 = "cause death: colorectal cancer (prov.)",
                      cause_death_2_1_8 = "cause death: lung cancer (prov.)",
