...
sez2011_industry.df <- 
  sez2011_industry.df %>%
  dplyr::select(-manufacturing, -construction, -wholesaling, -retail, -transport, -warehousing, -delivery, -hospitality, -services, -education, -healthcare, -agedcare)
...
hex_industry.df <- 
  ...
  dplyr::summarize(n_companies = n(),
                   ...
                   sum_manufacturing = 
                     sum(ADDETTI[ATECO3_code == "manufacturing"]) + 
                     sum(ALTRI_RETRIB[ATECO3_code == "manufacturing"]) + 
                     sum(VOLONTARI[ATECO3_code == "manufacturing"]),
                   sum_other = 
...
ggplot(hex_ita_10km.sf) +
  geom_sf(aes(fill = log(sum_manufacturing / P1)+1), colour = NA) +
  scale_fill_viridis()
```
