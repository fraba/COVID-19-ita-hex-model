...
hex_ita_1km.sf <- 
  st_make_grid(ita_buffer.sf,
               ...
               square = FALSE) %>%
  st_sf(hex_id = 0)
hex_ita_1km.sf$hex_id <- 1:nrow(hex_ita_1km.sf)
```

```{r}
ggplot(hex_ita_1km.sf) + geom_sf()
```
...
hex_population.df <- 
  ...
  dplyr::mutate_at(vars(CODREG:PROCOM), function(x) names(which.max(table(x)))) %>%
  dplyr::mutate_at(vars(P1:E31), sum, na.rm = TRUE) %>%
  dplyr::distinct()

hex_ita_1km.sf <-
  merge(hex_ita_1km.sf, 
        hex_population.df, 
        by = "hex_id",
        all.x = T)
```
...
```{r}
save(hex_ita_1km.sf, file = "hex_ita_1km.sf.RData")
```
